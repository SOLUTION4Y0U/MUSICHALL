...
const Home = () => {
  ...
  return (
    <div className="min-h-screen bg-brand-black">
      {/* Hero Section */}
      <div className="relative bg-gradient-to-br from-brand-black via-brand-dark to-brand-mid-gray/20 text-brand-light-gray py-20 px-[18px] text-center overflow-hidden">
        ...
        <div className="relative z-10 max-w-2xl mx-auto">
          <h1 className="font-secondary text-4xl md:text-5xl font-bold text-brand-white mb-6 drop-shadow-lg">
            ONEENERGY
          </h1>
          <p className="text-xl mb-8 text-brand-light-gray/90 leading-relaxed">
            международный поставщик мобильных телефонов, портативной электроники и бытовой техники
          </p>
          <Link
            to={ROUTES.CATALOG}
            className="btn-primary inline-block text-lg shadow-lg transform hover:scale-105"
          >
            Открыть каталог
          </Link>
        </div>
      ...
      {/* Categories Section */}
<section className="py-16 px-[18px]">
  <div className="max-w-6xl mx-auto">
    <h2 className="font-secondary text-3xl font-bold text-brand-white mb-4">
      Наши бренды
    </h2>
    <div className="w-20 h-1 bg-gradient-to-r from-brand-copper to-transparent mb-8"></div>

    {/* Slider Container */}
    <div className="relative overflow-hidden">
      <div
        ref={sliderRef}
        className="flex snap-x snap-mandatory overflow-x-auto no-scrollbar" /* Добавлен no-scrollbar */
      ...
      >
        {categories.slice(0, 4).map((category, index) => (
          <div
            key={category.id}
            className="flex-shrink-0 w-full snap-start px-2" /* Добавлен px-2 для отступов */
          >
            <Link
            ...
            >
              ...
              <div className="absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-black/90 to-transparent">
                <h3 className="text-2xl font-bold text-brand-white group-hover:text-brand-copper transition-colors duration-300">
                  {category.name}
                </h3>
              </div>
            </Link>
    ...
    <div className="text-center mt-12">
      ...
      <section className="space-y-6">
        ...
        <div className="text-center mt-8">
          <Link
            to={ROUTES.CATALOG}
            className="inline-flex items-center space-x-2 text-brand-white border-2 border-brand-copper px-6 py-3 rounded-lg hover:bg-brand-copper hover:text-brand-white transition-all duration-300 font-medium"
          >
