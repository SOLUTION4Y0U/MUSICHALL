...
import ProductCardPhilips from '../ProductCard/product_card_philips';
...
const ProductListPhilips: FC<ProductListProps> = ({ products, loading }) => {
  if (loading) {
    return (
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
      {products.map(product => (
        <ProductCardPhilips key={product.id} product={product} />
      ))}
    </div>
  );
};

export default ProductListPhilips;