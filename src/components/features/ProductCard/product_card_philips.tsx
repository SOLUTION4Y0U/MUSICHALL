...
const ProductCardPhilips: FC<ProductCardProps> = ({ product }) => {
  ...
  return (
    ...
    >
      ...
      <div className="p-4 space-y-3 bg-gradient-to-b from-black/60 to-transparent">
        <h3 className="font-medium text-lg text-brand-white group-hover:text-brand-blue transition-colors duration-300 line-clamp-2">
          {product.title}
        </h3>
        ...
        <button
          ...
          className={`w-full py-2 px-4 rounded-md font-medium transition-all duration-300 mt-2 ${
            product.stock === 0
              ...
              : inCart
                ? 'bg-blue-500/90 backdrop-blur-sm text-white hover:bg-blue-600/90'
                : 'bg-brand-blue/90 backdrop-blur-sm text-white hover:bg-brand-blue-dark'
          }`}
        >
          {product.stock === 0
            ? 'Нет в наличии'
...
export default ProductCardPhilips;